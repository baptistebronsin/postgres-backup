...
pg_dump_output=$(PGPASSWORD=${DB_PASSWORD} pg_dump -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -F p -c --if-exists --no-owner -v --column-inserts -f ${ABSOLUTE_BACKUP_FILE} 2>&1)
