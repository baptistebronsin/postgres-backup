...
pg_dump_output=$(PGPASSWORD=${DB_PASSWORD} pg_dump -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -F p -c -v -f ${BACKUP_FILE} 2>&1)
...
s3cmd_put_output=$(s3cmd put ${BACKUP_FILE} s3://${S3_BUCKET}/${BACKUP_FILE} \
